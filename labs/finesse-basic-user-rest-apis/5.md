## Step 5: Sign out the user from Finesse

> Finesse API for signing out a user
>  ```http
http://64.103.26.58:8082/finesse/api/User/{id}
```
> For more details, see <a href="https://developer.cisco.com/media/finesseDevGuide2/CFIN_RF_SC0D86B5_00_sign-out-of-finesse.html" target="_blank">User - Sign Out of Finesse</a>
 ...
 * In the large body text box, paste the XML user data as shown above. Change the value of the extension tag to the extension of the user in the ``/User`` resource
    ...
    ![User PUT Request Logout](/posts/files/finesse-basic-user-rest-apis/assets/images/user-PUT-request-logout.jpg)
...
3. If the request is successful, you should see a HTTP status of 202 Accepted and an empty Body textbox
 ...
 ![User PUT Response Logout](/posts/files/finesse-basic-user-rest-apis/assets/images/user-PUT-response-logout.jpg)
4. To confirm that the user was logged in successfully, use the GET user API from the **Step 2: Get the user information section** and verify that the ``<state>`` tag value is LOGOUT. If you kept the tab from Step 2 opened, switch back to that tab and click the **Send** button.

 ![User PUT Request After Logout](/posts/files/finesse-basic-user-rest-apis/assets/images/user-GET-response-after-logout.jpg)