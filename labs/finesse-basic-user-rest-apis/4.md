...
4. To confirm that the user was logged in successfully, use the GET user API from the **Step 2: Get the user information section** and verify that the <state> tag value is READY. If you kept the tab from Step 2 opened, switch back to that tab and click the **Send** button.
 ...
 5. Repeat steps 1-4 in this section to change the user’s state to NOT_READY by using the XML user data to change the user state to NOT_READY as shown above. The reason why you need to change the state to NOT_READY is because you will be signing out the user in the next step. A user cannot sign out from READY state.
